...
#define ND_PASS_SHADER "ndpass"
#define LP_PASS_SHADER "lppass"
#define FR_PASS_SHADER "frpass"
...
//Next 3 constants define shader source prefixes for eavh pass
#define ND_SHADER_PREFIX ""
#define LPP_SHADER_PREFIX ""
#define FR_SHADER_PREFIX ""
...
bool GlobeApp::resetValues() {
	...
	m_fvEyePos[POS_ATTR_LOC] = (GLfloat) 0.000F;
	m_fvEyePos[NOR_ATTR_LOC] = (GLfloat) 0.000F;
	m_fvEyePos[TAN_ATTR_LOC] = (GLfloat) 5.000F;
	m_fvEarthPos[POS_ATTR_LOC] = (GLfloat) 0.000F;
	m_fvEarthPos[NOR_ATTR_LOC] = (GLfloat) 0.000F;
	m_fvEarthPos[TAN_ATTR_LOC] = (GLfloat) 0.000F;
	m_fvSunPos[POS_ATTR_LOC] = (GLfloat) -4.000F;
	m_fvSunPos[NOR_ATTR_LOC] = (GLfloat) 4.000F;
	m_fvSunPos[TAN_ATTR_LOC] = (GLfloat) 6.000F;
	m_fvAmbColor[POS_ATTR_LOC] = (GLfloat) 0.005F;
	m_fvAmbColor[NOR_ATTR_LOC] = (GLfloat) 0.005F;
	m_fvAmbColor[TAN_ATTR_LOC] = (GLfloat) 0.010F;
	m_fvBgColor[POS_ATTR_LOC] = (GLclampf) 0.005F;
	m_fvBgColor[NOR_ATTR_LOC] = (GLclampf) 0.005F;
	m_fvBgColor[TAN_ATTR_LOC] = (GLclampf) 0.000F;
	...
	m_fvUp[POS_ATTR_LOC] = GLFLOAT_ZERO;
	m_fvUp[NOR_ATTR_LOC] = GLFLOAT_ONE;
	m_fvUp[TAN_ATTR_LOC] = GLFLOAT_ZERO;
	for (int k = POS_ATTR_LOC; k < BIN_ATTR_LOC; k++) {
	...
	NvBuildTranslateMatf(m_fmModel, m_fvEarthPos[POS_ATTR_LOC],
			m_fvEarthPos[NOR_ATTR_LOC], m_fvEarthPos[TAN_ATTR_LOC]);
	NvBuildTranslateMatf(m_fmInvModel, -m_fvEarthPos[POS_ATTR_LOC],
			-m_fvEarthPos[NOR_ATTR_LOC], -m_fvEarthPos[TAN_ATTR_LOC]);
...
bool GlobeApp::initTexturesGL(void) {
	bool tex0 = createHHDDSTextureGL(m_uivTextures[0], 0,
			GL_TEXTURE0, true, true, COLOR_TEXTURE_FILENAME);
	bool tex1 = createNormalMapGL(m_uivTextures[1], 1,
			GL_TEXTURE1, m_fHeightScale, HEIGHTMAP_TEXTURE_FILENAME);
	bool tex2 = createHHDDSTextureGL(m_uivTextures[2], 2,
			GL_TEXTURE2, true, true, CITYLIGHTS_TEXTURE_FILENAME);
	bool tex3 = createHHDDSTextureGL(m_uivTextures[3], 3,
			GL_TEXTURE3, true, true, CLOUDS_TEXTURE_FILENAME);
	return tex0 && tex1 && tex2 && tex3;
}

/*
 * Shader initialiazation for deferred lighting
 *
 * 	We have to add new shader programs for each pass:
 * 		1. Normal + depth pass - ndpass
 * 		2. Lighting pre-pass - lppass
 * 		3. Forward pass - frpass
 */

bool GlobeApp::initLppShaderGL(void) {
	char ucBuffer[1024];
	memset(ucBuffer, '\0', 1024);

	//TODO prefix Normal + depth pass shader
	sprintf(ucBuffer, ND_SHADER_PREFIX);

	m_uivNDPassProgram[0] = nv_load_program(NDPASS, ucBuffer);

	glBindAttribLocation(m_uivNDPassProgram[0], POS_ATTR_LOC, "pos_attr");
	glBindAttribLocation(m_uivNDPassProgram[0], NOR_ATTR_LOC, "nor_attr");
	glBindAttribLocation(m_uivNDPassProgram[0], TAN_ATTR_LOC, "tan_attr");
	glBindAttribLocation(m_uivNDPassProgram[0], BIN_ATTR_LOC, "bin_attr");
	glBindAttribLocation(m_uivNDPassProgram[0], TEX_ATTR_LOC, "tex_attr");

	glLinkProgram(m_uivNDPassProgram[0]);

	glUseProgram(m_uivNDPassProgram[0]);

	//Uniformes
	m_iMVPMatLoc = glGetUniformLocation(m_uivNDPassProgram[0], "MVPMat");
	m_iModelMatLoc = glGetUniformLocation(m_uivNDPassProgram[0], "ModelMat");
	m_iModelViewMatLoc = glGetUniformLocation(m_uivNDPassProgram[0],
			"ModelViewMat");

	//Set Normal map Tex
	glUniform1i(glGetUniformLocation(m_uivNDPassProgram[0], "NormalMapTex"),
			(GLint) 1);
}
...
bool GlobeApp::initShadersGL(void) {
	...
	for (int k = POS_ATTR_LOC; k < (int) dimof(m_uivGlobePrograms); k++) {
		//Fill in the #defines to be included in the shader source code before
		//what is in the text file
		memset(ucBuffer, '\0', 1024);
		sprintf(ucBuffer, GLOBE_SHADER_SOURCE,
				(k >> 2) & 1,
				(k >> 1) & 1, k & 1,
				m_fvAmbColor[0], m_fvAmbColor[1],
				m_fvAmbColor[2], m_fParallaxScale, m_fParallaxBias);
		m_uivGlobePrograms[k] = nv_load_program(GLOBE_SHADER, ucBuffer);

		if (k >= TEX_ATTR_LOC) {
			glBindAttribLocation(m_uivGlobePrograms[k], 0,
					"raw_attr");
		} else {
			glBindAttribLocation(m_uivGlobePrograms[k], 1,
					"pos_attr");
			glBindAttribLocation(m_uivGlobePrograms[k], 2,
					"nor_attr");
			glBindAttribLocation(m_uivGlobePrograms[k], 3,
					"tan_attr");
			glBindAttribLocation(m_uivGlobePrograms[k], 4,
					"bin_attr");
		...
		m_iRadiusUnifLocGlobe[k] = glGetUniformLocation(m_uivGlobePrograms[k],
				"Radius");
		//Added as a deferred lighting test
		m_iModelMatUnifLocGlobe[k] = glGetUniformLocation(m_uivGlobePrograms[k],
				"ModelMat");
...
//
//		//Uniforms that need to be updated every frame
//		m_iMVPMatUnifLocCloud[k] =
//			glGetUniformLocation(m_uivCloudPrograms[k], "MVPMat");
//		m_iSunPosObjUnifLocCloud[k] =
//			glGetUniformLocation(m_uivCloudPrograms[k], "SunPosObj");
//		m_iEyePosObjUnifLocCloud[k] =
//			glGetUniformLocation(m_uivCloudPrograms[k], "EyePosObj");
//		m_iRadiusUnifLocCloud[k] =
//			glGetUniformLocation(m_uivCloudPrograms[k], "Radius");
//
//		//Uniforms that need to be set once
//		glUniform1i(glGetUniformLocation(m_uivCloudPrograms[k],
//					"CloudsTex"), (GLint)3);
//	}
...
bool GlobeApp::initStateGL(void) {
	...
	glClearColor(m_fvBgColor[0], m_fvBgColor[1],
			m_fvBgColor[2], (GLclampf) 1.0f);
...
bool GlobeApp::resizeGL() {
	glViewport((GLint) 0, (GLint) 0,
			(GLsizei) (m_iWinWidth), (GLsizei) (m_iWinHeight));
	...
	DEBUG("GlobalAppState::update:\t(zNear, zFar): (%.2f, %.2f)", m_fZNear,
			m_fZFar);
...
void GlobeApp::drawGeometry() {
	GLfloat fmCurrModel[4][4],
			fmInvCurrModel[4][4];
	GLfloat fmMVP[4][4];
	...
	//ModelView matrix
	NvMultMatf(m_fmModelView, m_fmView, fmCurrModel);

	//6. DRAW GLOBE (in VBOs)
	glUseProgram(m_uivNDPassProgram[0]);
	glUniformMatrix4fv(m_iMVPMatUnifLocGlobe[m_uiCurrGlobeProgIdx],
			(GLsizei) 1, GL_FALSE,
			&(fmMVP[0][0]));
	//Added as a deferred lighting test
	glUniformMatrix4fv(m_iModelMatUnifLocGlobe[m_uiCurrGlobeProgIdx],
			(GLsizei) 1, GL_FALSE,
			&(fmCurrModel[0][0]));
	glUniform3fv(m_iModelViewMatLoc,
			(GLsizei) (1), &(m_fmModelView[0][0]));

	glUniform3fv(m_iEyePosObjUnifLocGlobe[m_uiCurrGlobeProgIdx],
...
void GlobeApp::updateAnimation(const float current_time) {
	if (!m_bPauseAnim) {
...
bool GlobeApp::renderGL(const float current_time) {
	GLfloat fmCurrModel[TEX_ATTR_LOC][TEX_ATTR_LOC],
				fmInvCurrModel[TEX_ATTR_LOC][TEX_ATTR_LOC];
		GLfloat fmMVP[TEX_ATTR_LOC][TEX_ATTR_LOC];
		GLfloat m_fvSunPosObj[BIN_ATTR_LOC], m_fvEyePosObj[BIN_ATTR_LOC];
		//We use the same IBO for all VBOs, bind it once
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_uiIBO);
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, m_uivTextures[POS_ATTR_LOC]);
		glActiveTexture(GL_TEXTURE1);
		glBindTexture(GL_TEXTURE_2D, m_uivTextures[NOR_ATTR_LOC]);
		glActiveTexture(GL_TEXTURE2);
		glBindTexture(GL_TEXTURE_2D, m_uivTextures[TAN_ATTR_LOC]);
		glActiveTexture(GL_TEXTURE3);
		glBindTexture(GL_TEXTURE_2D, m_uivTextures[BIN_ATTR_LOC]);
		glActiveTexture(GL_TEXTURE0);
		glEnable(GL_BLEND);
		//2. MODEL MATRIX
		//2.1. Create current model matrix (from stored matrix with translation)
		...
		glUniformMatrix4fv(m_iMVPMatUnifLocGlobe[m_uiCurrGlobeProgIdx],
				(GLsizei) NOR_ATTR_LOC, GL_FALSE,
				&(fmMVP[POS_ATTR_LOC][POS_ATTR_LOC]));
		//Added as a deferred lighting test
		glUniformMatrix4fv(m_iModelMatUnifLocGlobe[m_uiCurrGlobeProgIdx],
				(GLsizei) NOR_ATTR_LOC, GL_FALSE,
				&(fmCurrModel[POS_ATTR_LOC][POS_ATTR_LOC]));
		glUniform3fv(m_iSunPosObjUnifLocGlobe[m_uiCurrGlobeProgIdx],
				(GLsizei) (NOR_ATTR_LOC), m_fvSunPosObj);
		glUniform3fv(m_iEyePosObjUnifLocGlobe[m_uiCurrGlobeProgIdx],
				(GLsizei) (NOR_ATTR_LOC), m_fvEyePosObj);
		glUniform1f(m_iRadiusUnifLocGlobe[m_uiCurrGlobeProgIdx], m_fGlobeRadius);
		if (m_uiCurrGlobeProgIdx < 4U) {
			glBindBuffer(GL_ARRAY_BUFFER, m_uiGlobeVBO);
...
bool GlobeApp::switchProgramGL(void) {
	...
	DEBUG(
		   ...
		   Using globe program %d and cloud program %d",
			m_bRawData, m_bParallaxMapping, m_bNormalMapping,
			m_uiCurrGlobeProgIdx, m_uiCurrCloudProgIdx);
...
bool GlobeApp::cleanupGL(void) {
	...
	for (int k = POS_ATTR_LOC; k < (int) dimof(m_uivTextures); k++) {
		if (m_uivTextures[k] && glIsTexture(m_uivTextures[k])) {
			glDeleteTextures((GLsizei) NOR_ATTR_LOC, &(m_uivTextures[k]));
	...
	for (int k = POS_ATTR_LOC; k < (int) dimof(m_uivCloudPrograms); k++) {
	...
	if (m_uiGlobeVBO && glIsBuffer(m_uiGlobeVBO)) {
		glDeleteBuffers((GLsizei) NOR_ATTR_LOC, &(m_uiGlobeVBO));
		m_uiGlobeVBO = GLUINT_ZERO;
	} else {
	...
	if (m_uiCloudVBO && glIsBuffer(m_uiCloudVBO)) {
		glDeleteBuffers((GLsizei) NOR_ATTR_LOC, &(m_uiCloudVBO));
		m_uiCloudVBO = GLUINT_ZERO;
	} else {
	...
	if (m_uiRawVBO && glIsBuffer(m_uiRawVBO)) {
		glDeleteBuffers((GLsizei) NOR_ATTR_LOC, &(m_uiRawVBO));
		m_uiRawVBO = GLUINT_ZERO;
	} else {
	...
	if (m_uiIBO && glIsBuffer(m_uiIBO)) {
		glDeleteBuffers((GLsizei) NOR_ATTR_LOC, &(m_uiIBO));
		m_uiIBO = GLUINT_ZERO;
	} else {
...
bool GlobeApp::initNDMeshes(void) {
	FullVertex* globeVerets = NULL;
	GLushort* elements = NULL;

	//Get information for the globe VBO, the raw VBO and the IBO
	if (!(createSphere(&globeVerets, NULL, &elements, &(m_uiNumVertices),
			&(m_uiNumIndices), m_fGlobeRadius, m_uiVertSlices)) || !usvElements)
		return false;
...
bool GlobeApp::renderG(void) {
	GLuint framebuffer;
	GLuint depthRenderbuffer;
	GLuint texture;
	GLint texWidth = SCREEN_W, texHeight = SCREEN_H;
